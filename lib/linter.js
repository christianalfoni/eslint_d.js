...
function translateOptionsCLIEngine(cliOptions, cwd) {
  return {
    ...
    fix: cliOptions.fixDryRun
      ? false
      : cliOptions.fix || cliOptions.fixToStdout,
...
function translateOptionsESLint(cliOptions, cwd) {
  return {
    ...
    overrideConfig: {
      env:
        cliOptions.env &&
        cliOptions.env.reduce((obj, name) => {
          obj[name] = true;
          return obj;
        }, {}),
      globals:
        cliOptions.global &&
        cliOptions.global.reduce((obj, name) => {
...
function executeWithCLIEngine(CLIEngine, cwd, opts, text, callback) {
  ...
  if (opts.printConfig) {
    if (files.length !== 1) {
      callback(
        "The --print-config option requires a single file as positional " +
          "argument."
      );
      return;
    }
...
exports.invoke = async function (cwd, args, text, callback) {
  ...
  try {
    ...
    // When ESLint can not be resolved it throws
  }
  ...
  const options = cache.eslint.ESLint ? options_eslint : options_cliengine;
